...
def add(request):
	if request.user.is_authenticated():
		...
		if typeadd == 'url':
			...
			new_challenge = Challenge(name = name, creator = username, description = description, category = category, level = difficulty, type_upload = typeadd, status = 'prive', process = 'non_valide', clue = indice, url = challenge, points = '10')
			new_challenge.save()
		elif typeadd == 'zip':
			typeadd = '2'
			...
			new_challenge = Challenge(name = name, creator = username, description = description, category = category, level = difficulty, type_upload = typeadd, status = 'prive', process = 'non_valide', clue = indice, file = fichier, url = url, points = '10')
			new_challenge.save()
		else:
			typeadd = '3'
