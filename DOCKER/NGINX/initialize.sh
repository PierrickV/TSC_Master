...
if [ "$DJANGO_PRODUCTION" != "true" ]; then
    ...
    if [ "$ROOT_PASSWORD" == "" ]; then
        ...
        echo "from ConfigParser import SafeConfigParser; parser = SafeConfigParser(); parser.read('/code/config.ini'); from django.contrib.auth.models import User; print 'Root user already exists' if User.objects.filter(username='root') else User.objects.create_superuser('root', 'veran@intechinfo.fr', parser.get('general', 'ROOT_PASSWORD'))" | python /code/django_docker/manage.py shell
    else
        # Root password environment variable IS set; so, use it
        echo "import os; from django.contrib.auth.models import User; print 'Root user already exists' if User.objects.filter(username='root') else User.objects.create_superuser('root', 'veran@intechinfo.fr', os.environ['ROOT_PASSWORD'])" | python /code/django_docker/manage.py shell
    fi
else
